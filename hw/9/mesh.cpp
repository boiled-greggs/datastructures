...
bool Mesh::Collapse() {
  ...
  // we need to get all the triangles associated with a and b
  triangles_set changing;
  std::set<Triangle*> a_tri = a->getTriangles();
  std::set<Triangle*> b_tri = b->getTriangles();
  std::set<Triangle*>::iterator itr;
  for (itr = a_tri.begin(); itr != a_tri.end(); ++itr)
    changing.insert(*itr);
  for (itr = b_tri.begin(); itr != b_tri.end(); ++itr)
    changing.insert(*itr);
  // now iterate through the set of changing triangles.
  // if it's one of the possible two triangles that need to
  // be removed, the vertices are not saved. otherwise,
  // the vertices involved will be saved and then the triangles
  // removed.
  std::vector<std::vector<Vertex*> > vertices;
  triangles_set::iterator t_itr = changing.begin();
  while (t_itr != changing.end()) {
    Triangle* tri = *t_itr;
    ++t_itr;
    if (tri->HasVertex(a) && tri->HasVertex(b)) {
      //++t_itr;
      // need to remove
      RemoveEdge(tri->getVertex(0), tri->getVertex(1));
      RemoveEdge(tri->getVertex(0), tri->getVertex(2));
      RemoveEdge(tri->getVertex(1), tri->getVertex(2));
      RemoveTriangle(tri);
    } else {
      //++t_itr;
      // need to save then remove
      std::vector<Vertex*> tmp;
      for (int i = 0; i < 3; ++i) {
        // change b vertices to a
        if (tri->getVertex(i) == b)
          tmp.push_back(a);
        else if (tri->getVertex(i) == a)
          tmp.push_back(a);
        else
          tmp.push_back(tri->getVertex(i));
      }
  ...
  // add remaining edited triangles back to the mesh
  for (int i = 0; i < vertices.size(); ++i)
    AddTriangle(vertices[i][0], vertices[i][1], vertices[i][2]);
...
void Mesh::AddEdge(Vertex *a_, Vertex *b_) {
  ...
  if (itr == edges.end()) {
    ...
    //
    // just need to push on the queue
...
void Mesh::RemoveEdge(Vertex *a_, Vertex *b_) {
  ...
  if (itr != edges.end()) {
    ...
    // ASSIGNMENT: additional code needed for priority queue
    //
    // just need to remove from the queue :)
    if (method == "priority_queue")
...
void Mesh::ReCalculateEdges(const std::set<Edge*>& r_e)  {
  for (std::set<Edge*>::iterator itr = r_e.begin(); itr != r_e.end(); itr++) {
    ...
    // ASSIGNMENT: additional code needed for priority queue
    //
    // need to reorder the priority queue because values have been changed
    if (method == "priority_queue")
...
Edge* Mesh::FindEdge() const {
  ...
  // these two different methods are handled in the edge
    ...
    // perform a priority queue to find the best edge
      ...
      // 
      // the right edge is at the top of the queue by definition
      answer = edges_pq.top();
    }
    else {
