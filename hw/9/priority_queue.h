...
void PriorityQueue<T>::remove(T element) {
  // use the locations map to find the location of the element
  // then move the element to the back of m_heap and remove it
  typename std::map<T, int>::iterator itr = locations.find(element);
  if (itr != locations.end()) {
    ...
    locations[m_heap[index]] = index;
    locations.erase(itr);

    // finally percolate up and down
    this->percolate_up(index);
...
void PriorityQueue<T>::percolate_up(int i) {
  // check for a parent first. while there's still a parent above,
  // update the locations map and swap values in the heap, so long
  // as the priority allows it
  while ( (i-1)/2 >= 0 && ( i > 0 && i < (int)size() ) ) {
...
void PriorityQueue<T>::percolate_down(int i) {
  // while there is a right or left child, move down through
  // the children. prefer moving through right children. once
  // have child, compare child to current value, and if priority
  // allows, swap
  while (has_right_child(i) || has_left_child(i)) {
    int child_ind = -1;
    // this if statement is a little complex, but checks if there is a right child
    // and if either 1) does not have a left child or, if there is a left child,
    // 2) the priority allows it, and gets the right child if it works out
    if ( has_right_child(i) && ( (!has_left_child(i) )  || 
        ( m_heap[get_right_child(i)]->getPriorityValue() < m_heap[get_left_child(i)]->getPriorityValue() ) ) ) {
      child_ind = get_right_child(i);
    } else if (has_left_child(i)) { // otherwise the child is the left child
      child_ind = get_left_child(i);
    }
    // perform the swap
    if (m_heap[child_ind]->getPriorityValue() < m_heap[i]->getPriorityValue()) {
      locations[m_heap[child_ind]] = i;
      locations[m_heap[i]] = child_ind;
      T tmp = m_heap[i];
      m_heap[i] = m_heap[child_ind];
      m_heap[child_ind] = tmp;
      i = child_ind; 
    } else 
      break;
  }
