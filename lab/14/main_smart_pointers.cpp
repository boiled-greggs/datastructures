...
class smartBalloon {
  public:
    smartBalloon(const std::string &n) : name(n) {
      std::cout << "Balloon constructor " << name << std::endl;
      num_ropes = 0;
      ropes = new dsSharedPtr<smartBalloon>[MAX_NUM_ROPES];
    }
    ~smartBalloon() {
      std::cout << "Balloon destructor " << name << std::endl;
      delete [] ropes;
    ...
    int numRopes() const {return num_ropes;}
    const dsSharedPtr<smartBalloon> getRope(int i) const {return ropes[i];}
    ...
    void addRope(dsSharedPtr<smartBalloon> b) {
      assert (num_ropes < MAX_NUM_ROPES);
      ropes[num_ropes] = b;
      ++num_ropes;
    }
    
    void removeRope(dsSharedPtr<smartBalloon> b) {
      for (int i = 0; i < MAX_NUM_ROPES; ++i) {
        if (ropes[i] == b) {
  ...
  private:
    std::string name;
    int num_ropes;
    dsSharedPtr<smartBalloon>*ropes;
};
// ====================================================
// ====================================================
...
*/
  ...
  //
  dsSharedPtr<smartBalloon> georgette2(new smartBalloon("Mr Potato Head"));
  dsSharedPtr<smartBalloon> henry2(new smartBalloon("Snoopy"));

  georgette2->addRope(henry2);
  henry2 = new smartBalloon("Tigger");
  georgette2->addRope(henry2);
  georgette2->print();
  henry2->print();
  
  dsSharedPtr<smartBalloon> isabelle2(new smartBalloon("Shrek"));
  henry2->addRope(isabelle2);
  isabelle2 = new smartBalloon("Barney the Purple Dinosaur");
