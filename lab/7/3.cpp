...
void print_grid(vector<vector<bool> > const& blocked_grid, std::vector<Point> const& correct_path, int start_x, int start_y) {

  if (correct_path.size() == 0)  cout << "Here is the grid with the origin in the upper left corner, x increasing \n"
  ...
  for (int y=0; y<blocked_grid[0].size(); ++y) {
    for (int x=0; x<blocked_grid.size(); ++x) {
      ...
      for (int i = 0; i < correct_path.size(); ++i) {
        if (correct_path[i].x == x && correct_path[i].y == y) on_path = true;
      }
...
void print_path(const std::vector<Point> &correct_path) {
  std::cout << "\nPath of points to the origin:\n";
  for (unsigned int i = 0; i < correct_path.size(); ++i) {
    std::cout << " ( " << correct_path[i].x << " , " << correct_path[i].y << " )\n";
  }
}

int find_origin(vector<vector<bool> > const& blocked_grid, int x, int y, std::vector<Point> &correct_path, std::vector<Point> scratch) { 
  scratch.push_back(Point(x,y));
  if (x == 0 && y == 0) {
    ...
    int zig = 0;
    int zag = 0;
    if (x > 0 && !blocked_grid[x-1][y]) zig = find_origin(blocked_grid, x-1, y, correct_path, scratch);
    if (y > 0 && !blocked_grid[x][y-1]) zag = find_origin(blocked_grid, x, y-1, correct_path, scratch);
    return zig + zag;
  }
}
...
int main(int argc, char* argv[]) {
  ...
  std::vector<Point> correct_path, scratch;
  int start_x, start_y;
  read_grid(istr, blocked_grid, start_x, start_y);
  print_grid(blocked_grid, correct_path, start_x, start_y);


  // Start here with your code...
  int paths = 0;
  paths = find_origin( blocked_grid, start_x, start_y, correct_path, scratch); //, paths, correct_path );
 
  print_path(correct_path);
  std::cout << "\ngrid with path drawn with $:\n\n";
  print_grid(blocked_grid, correct_path, start_x, start_y);
  std::cout << std::endl;
