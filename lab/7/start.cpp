...
int find_origin(vector<vector<bool> > const& blocked_grid, int x, int y) { //, int &count, std::vector<Point> correct_path){
	if (x == 0 && y == 0) {
    return 1;
  } else if (blocked_grid[x][y]) {
    return 0;
    ...
    int zig = 0;
    int zag = 0;
    if (x > 0) zig = find_origin(blocked_grid, x-1, y);
    if (y > 0) zag = find_origin(blocked_grid, x, y-1);
    return zig + zag;
  }
}
...
int main(int argc, char* argv[]) {
  ...
  int paths = 0;
  // std::vector<Point> correct_path;
  paths = find_origin( blocked_grid, start_x, start_y); //, paths, correct_path );
  std::cout << "\n Number of paths available: " << paths << "\n" << std::endl;
