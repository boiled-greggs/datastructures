...
int main() {
  ...
  std::cout << std::endl;
  std::cout << "lab provided test cases, sorted: \n";
  std::vector<Time> labtimes;
  labtimes.push_back(Time(12, 0, 0));
  labtimes.push_back(Time(0, 0, 0));
  labtimes.push_back(Time(24, 0, 0));
  labtimes.push_back(Time(14, 4, 5));
  std::sort(labtimes.begin(), labtimes.end(), isEarlierThan);
  for (unsigned int i = 0; i < labtimes.size(); i++) {
    std::cout << "lab time " << labtimes[i].getHour() << " " << labtimes[i].getMinute() << " " << labtimes[i].getSecond() << ": ";
    labtimes[i].PrintAMPM();
  }
}
