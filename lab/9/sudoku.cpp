...
int Sudoku::Propagate(int i, int j) {
  int changed = 0;
  std::set<int>::iterator p = board[i][j].begin();
  int value = *p;
  for (int k = 0; k < size; ++k) {
    if (k!=j) {
      if (board[i][k].erase(value) == 1)
        ++changed;
    }
    if (k!=i) {
      if (board[k][j].erase(value) == 1)
        ++changed;
    }
  }
  int i_quad = i/quadrant_size;
  int j_quad = j/quadrant_size;
  for (int x = i_quad*quadrant_size; x < (i_quad+1)*quadrant_size; ++x) {
    for (int y = j_quad*quadrant_size; y < (j_quad+1)*quadrant_size; ++y) {
      if (x!=i && y!=j) {
        if (board[x][y].erase(value) == 1)
          ++changed;
      }
    }
  }
