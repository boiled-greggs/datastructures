...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_) {
    TreeNode<T> *new_ = new TreeNode<T>(old_->value);
    copy_helper( new_, old_);
    return new_;
  ...
  void copy_helper(TreeNode<T>* new_parent, TreeNode<T>* old_parent){
    if (old_parent != NULL){
      ...
      copy_helper(new_parent->right, old_parent->right);
      copy_helper(new_parent->left, old_parent->left);
    }
  }
...
/*
*/
  iterator find(const T& key_value, TreeNode<T>* p){   //non recursive
    TreeNode<T>* current_node = p;
    while(current_node != NULL) {
      if(current_node->value > key_value) {
        current_node = current_node->left;
      } else if (current_node->value < key_value) {
